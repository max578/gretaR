...
- **Native R torch backend** – automatic differentiation and zero Python
  dependency (GPU/MPS acceleration is on the roadmap)
- **Multi-chain batching** –
  `mcmc(..., sampler = "hmc", batched = TRUE)` advances all chains as
  one batched torch computation; wall-clock stays roughly flat in the
  number of chains (~2x at 8 chains, ~4x at 16 on CPU)
