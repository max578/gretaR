...
| **Laplace** | `laplace()` | Very fast | Gaussian approximation | Quick uncertainty around a mode |
...
```{r inference-compare, fig.cap = "Density overlays for the robust-regression intercept, slope and residual scale under NUTS. The chains agree closely with each other, giving the reference posterior the faster MAP, Laplace and ADVI approximations above are checked against."}
plot(mcmc_fit, type = "density")
```

NUTS gives the reference posterior here: MAP and the Laplace approximation
report a point estimate and a local Gaussian curvature around it, and ADVI
reports a factorised approximate posterior, so the three faster methods
are used to sanity-check a model quickly before committing to the full
NUTS run.
