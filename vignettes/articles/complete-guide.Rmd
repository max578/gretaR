---
title: "Complete Guide to gretaR"
---
...
Every distribution creates a variable node with an appropriate constraint
and transform for unconstrained sampling.
...
`gretaR_glm()` provides a high-level interface using standard R formulas.
...
Full mgcv spline syntax is supported via the `smooth2random` decomposition.
...
A smooth term combined with an `(1 | group)` random-effect term in the same
formula (`y ~ s(sx, k = 8) + (1 | group)`) is not currently supported --
`.gretaR_glm_mixed()`'s fixed-effects parsing does not preprocess `s()`
terms before building the model frame, and the call errors with `invalid
type (list) for variable`. Fit the smooth and the group effect as two
separate formula-interface or DSL models until that combination is
supported.
...
Finite mixtures use log-sum-exp marginalisation over discrete components.
`mixture()` accepts a `weights` argument built from `dirichlet()`, but
`dirichlet()` cannot currently be sampled as a latent variable -- its
simplex-constrained support has no bijective transform implemented yet
(planned for v0.3; see `?dirichlet`), and `model()` refuses any attempt to
treat it as a free parameter. Until that lands, mixture weights must be
supplied as fixed values (a plain numeric vector), with only the component
...
with known gaps, not a drop-in faster replacement**: the Stan code emitter
currently supports only the normal, bernoulli, poisson, gamma, student-t
and beta families, and a likelihood or prior outside that list is not
translated to Stan -- see `NEWS.md` for the current status before relying
on it for any model whose distributions are not all in that list. No
timing claim is made here; benchmark your own model on both backends
...
| Quick prototyping | `"torch"` + `opt()` | No compilation wait |
...
```{r diagnostics-trace, fig.cap = "Trace plot for the two-parameter normal model fitted in Section 16. Both chains occupy the same band throughout, with no drift and no chain stuck away from the other."}
library(bayesplot)
plot(fit, type = "trace")
```
...
```{r diagnostics-density, fig.cap = "Posterior density overlays for the same fit. The two chains' densities are near-identical unimodal curves, corroborating the trace plot's mixing signal."}
...
```{r diagnostics-pairs, fig.cap = "Bivariate posterior for mu and sigma. The scatter is a single elliptical cloud with no funnel or multimodal structure, which is what an identifiable two-parameter model looks like."}
plot(fit, type = "pairs")
```

A single elliptical scatter with no funnel or secondary mode confirms the
two parameters are jointly identified by this likelihood.

```{r diagnostics-rhat, fig.cap = "R-hat by parameter. Both bars sit at or just above 1, at or below the 1.05 convergence threshold quoted above."}
plot(fit, type = "rhat")
```
...
`random_effect()` (see the *Hierarchical Models* vignette) builds the
same group-level prior on a continuum between the non-centred and centred
forms via its `centring` argument, and is the recommended way to write a
group-level term -- hand-rolling either endpoint below is equivalent to
