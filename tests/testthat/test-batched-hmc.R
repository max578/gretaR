...
.draws_of <- function(raw) {
  s <- raw$samples; dimnames(s) <- list(NULL, NULL, raw$param_names)
...
test_that("batched HMC recovers a conjugate-Normal posterior (mean + sd + rhat)", {
  ...
  n <- 8L; sig <- 2; yv <- rnorm(n, 3, sig)
  pv <- 1 / (1 / 100 + n / sig^2); pm <- pv * sum(yv) / sig^2; psd <- sqrt(pv)

  reset_gretaR_env()
  y <- as_data(yv); mu <- normal(0, 10); distribution(y) <- normal(mu, sig)
...
test_that("batched HMC posterior agrees with single-chain HMC (A/B)", {
  ...
  nn <- 50; xx <- rnorm(nn); yy <- 1.5 - 0.8 * xx + rnorm(nn)
  build <- function() {
    reset_gretaR_env()
    X <- as_data(cbind(1, xx)); b <- normal(0, 5, dim = 2); s <- half_cauchy(2)
    eta <- X %*% b; yd <- as_data(yy); distribution(yd) <- normal(eta, s); model(b, s)
  }
  ...
  for (v in c("b[1]", "b[2]", "s")) {
    a <- sb[sb$variable == v, ]; b2 <- ss[ss$variable == v, ]
