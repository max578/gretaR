...
[![Codecov test coverage](https://codecov.io/gh/max578/gretaR/branch/main/graph/badge.svg)](https://app.codecov.io/gh/max578/gretaR?branch=main)
<!-- badges: end -->
...
gretaR will be available from CRAN after acceptance. In the meantime, install
binaries from R-Universe (built within ~1 hour of every commit to `main`):
...
After installing the package, set up the torch backend (one-time download of
LibTorch on first use):
