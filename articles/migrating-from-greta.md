# Migrating from greta to gretaR
...
1.  **No Python dependency.** gretaR uses [torch for
    R](https://torch.mlverse.org/) as its computation backend.
    Installation is a single `install.packages("gretaR")` call with no
    `reticulate`, no conda environments, and no TensorFlow version
    conflicts.

2.  **Broader inference methods.** In addition to HMC/NUTS, gretaR ships
    MAP optimisation
    ([`opt()`](https://max578.github.io/gretaR/reference/opt.md)),
    Laplace approximation
    ([`laplace()`](https://max578.github.io/gretaR/reference/laplace.md)),
    and automatic differentiation variational inference
    ([`variational()`](https://max578.github.io/gretaR/reference/variational.md)).
    These are useful for rapid prototyping, model comparison, and
    large-data settings where full MCMC is impractical.

3.  **Active, focused development.** gretaR is under active development
    with a clear roadmap covering sparse matrix support, formula-based
    model specification, and extended distribution families.
...
greta requires a working Python installation with TensorFlow and
TensorFlow Probability. This is the single most common source of
installation failures. gretaR replaces the entire Python stack with
[torch for R](https://torch.mlverse.org/), which is a native R package
...
Under the hood, gretaR builds a computation graph using torch tensors.
...
a high-level convenience function for fitting Bayesian generalised
...
greta uses names like [`gamma()`](https://rdrr.io/r/base/Special.html),
...
accepts sparse matrices from the **Matrix** package directly. This is
important for large, sparse design matrices common in ecology, genomics,
and spatial models.
...
greta returns `coda::mcmc.list` objects from
[`mcmc()`](https://max578.github.io/gretaR/reference/mcmc.md). gretaR
returns
[`posterior::draws_array`](https://mc-stan.org/posterior/reference/draws_array.html)
objects, which integrate directly with the modern **posterior** and
...
gretaR, all sampler options are direct arguments to
[`mcmc()`](https://max578.github.io/gretaR/reference/mcmc.md).
...
Replace truncated Cauchy/normal priors with
...
Replace `coda::` summary/plotting calls with `posterior::` /
`bayesplot::` equivalents

If using `greta::opt()` or `greta::calculate()`: check gretaR
equivalents
([`opt()`](https://max578.github.io/gretaR/reference/opt.md),
[`laplace()`](https://max578.github.io/gretaR/reference/laplace.md),
[`variational()`](https://max578.github.io/gretaR/reference/variational.md))

Remove `greta::install_greta_deps()` and any Python/conda setup from
your workflow
